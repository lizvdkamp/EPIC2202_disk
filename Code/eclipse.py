...
def make_plots(patches, light_curves, light_curve, model_time, time, flux, 
    ...
    plt.xlabel('x [$R_*$]', fontsize=22)
    plt.ylabel('y [$R_*$]', fontsize=22)
    ...
    plt.xlim(xlim2)
    plt.ylim(ylim2)
    plt.xlabel('Time [BJD - %i]' % (2457409 + dt), fontsize=22)
    plt.ylabel('Normalised Flux [-]', fontsize=22)
